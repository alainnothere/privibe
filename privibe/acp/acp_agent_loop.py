...
from privibe import VIBE_ROOT, __version__
from privibe.acp.acp_logger import acp_message_observer
from privibe.acp.exceptions import (
    ConfigurationError,
...
from privibe.acp.tools.base import BaseAcpTool
from privibe.acp.tools.session_update import (
    tool_call_session_update,
    tool_result_session_update,
)
from privibe.acp.utils import (
    ToolOption,
...
from privibe.core.agents.models import CHAT as CHAT_AGENT, BuiltinAgentName
...
from privibe.core.proxy_setup import (
    ProxySetupError,
    parse_proxy_command,
    set_proxy_var,
    unset_proxy_var,
...
class VibeAcpAgentLoop(AcpAgent):
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        #  - dev mode: `uv run privibe-acp`, ran from the project root
        #  - uv tool install: `privibe-acp`, similar to dev mode, but uv takes care of path resolution
        #  - bundled binary: `./privibe-acp` from binary location
        ...
        # So sys.executable is already /path/to/binary/privibe-acp.
        ...
        auth_methods: list[EnvVarAuthMethod | TerminalAuthMethod | AuthMethodAgent] = (
            [
                TerminalAuthMethod(
                    ...
                    description="Register your API Key inside Privibe",
                    field_meta={
                        "terminal-auth": {
                            "command": command,
                            "args": args,
        ...
        try:
            if not args:
                ...
                if value is not None:
                    ...
                    message = f"Set `{key}={value}` in ~/.privibe/.env\n\nPlease start a new chat for changes to take effect."
                else:
                    unset_proxy_var(key)
                    message = f"Removed `{key}` from ~/.privibe/.env\n\nPlease start a new chat for changes to take effect."
    ...
    async def load_session(
        ...
        **kwargs: Any,
        ...
        try:
            _, metadata = SessionLoader.load_session(session_dir)
        except Exception as e:
        ...
        agent_loop.messages.restore(session_dir)
        agent_loop.session_logger.resume_existing_session(session_id, session_dir)

        session = await self._create_acp_session(session_id, agent_loop)

        non_system_messages = [
            m for m in agent_loop.messages
            if m.role != Role.system and not m.injected
        ]
