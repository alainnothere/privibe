...
from privibe.core.tools.base import BaseTool, BaseToolConfig
from privibe.core.tools.mcp import MCPRegistry
from privibe.core.utils import name_matches
...
def _try_canonical_module_name(path: Path) -> str | None:
    """Extract canonical module name for privibe package files.
...
def _compute_module_name(path: Path) -> str:
    """Return canonical module name for privibe files, hash-based synthetic name otherwise."""
...
class ToolManager:
    ...
    def _iter_tool_classes(search_paths: list[Path]) -> Iterator[type[BaseTool]]:
        ...
        for base in search_paths:
            ...
            # Sorted so the tool order (and with it the tools array sent to
            # the model, part of the KV-cache prefix) is identical across
            # processes and machines given the same files.
