...
**Prefer `hashed_read` + `hashed_replace_line` or `hashed_replace_block` when editing specific lines** — those tools don't require you to reproduce the old text exactly, which avoids whitespace and indentation mismatch errors. Use `search_replace` when replacing patterns that appear in multiple locations or when you already have the exact text to match.
