...
from privibe.core.autocompletion.file_enumerator import (
    ASCII_CODEPOINT_LIMIT,
    IndexEntry,
    build_ascii_mask,
    enumerate_entries,
)
from privibe.core.autocompletion.fuzzy import fuzzy_match
...
class PathCompleter(Completer):
    ...
    def __init__(
        ...
        self._cached_entries: list[IndexEntry] | None = None
    ...
    def _collect_matches(self, text: str, cursor_pos: int) -> list[str]:
        ...
        file_index = self._get_entries()
        scored_matches = self._score_matches(file_index, context)
        return [path for path, _ in scored_matches]

    def _get_entries(self) -> list[IndexEntry]:
        # Read once per "@" session: built on first use after invalidate(),
        # reused for every subsequent keystroke while the mention is open.
        # The controller calls invalidate() when the mention is dismissed, so
        # the next "@" re-enumerates and reflects any filesystem changes.
        with self._cache_lock:
            if self._cached_entries is None:
                # "." is the project root the completer was launched in.
                try:
                    self._cached_entries = enumerate_entries(Path("."))
                except OSError:
