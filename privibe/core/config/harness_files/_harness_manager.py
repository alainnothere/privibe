...
from privibe.core.config.harness_files._paths import (
    GLOBAL_AGENTS_DIR,
    GLOBAL_PROMPTS_DIR,
    GLOBAL_SKILLS_DIR,
    GLOBAL_TOOLS_DIR,
)
from privibe.core.paths import AGENTS_MD_FILENAME, VIBE_HOME, walk_local_config_dirs_all
...
class HarnessFilesManager:
    ...
    project_scan_depth: int = field(default=0)
    ...
    def config_file(self) -> Path | None:
        ...
        if workdir is not None:
            candidate = workdir / ".privibe" / "config.toml"
    ...
    def _walk_project_dirs(
        ...
        if workdir is None:
            return ((), (), ())
        return walk_local_config_dirs_all(workdir, max_depth=self.project_scan_depth)
    ...
    def project_prompts_dirs(self) -> list[Path]:
        ...
        if workdir is None:
            return []
        candidate = workdir / ".privibe" / "prompts"
    ...
    def load_extra_instruction_files(
        self, paths: list[Path]
    ) -> list[tuple[Path, str]]:
        """Load extra instruction files, resolving relative paths against cwd.

        Silently skips paths that don't exist or are empty.
        """
        results: list[tuple[Path, str]] = []
        for p in paths:
            resolved = p if p.is_absolute() else self._effective_cwd / p
            try:
                stripped = read_safe(resolved).strip()
                if stripped:
                    results.append((resolved, stripped))
            except (FileNotFoundError, OSError):
                pass
        return results
...
def init_harness_files_manager(*sources: FileSource) -> None:
    ...
    _manager = HarnessFilesManager(sources=sources, project_scan_depth=_project_scan_depth)


def set_project_scan_depth(depth: int) -> None:
    """Update the scan depth used by the harness manager.

    Called after config is loaded (the manager is initialized before config,
    so depth is applied here and the manager is recreated if needed).
    """
    global _manager, _project_scan_depth
    _project_scan_depth = depth
    if _manager is not None:
        _manager = HarnessFilesManager(
            sources=_manager.sources, project_scan_depth=depth
        )
