...
from privibe.core.paths import (
    AGENTS_MD_FILENAME,
...
def has_trustable_content(path: Path, *, max_depth: int = 0) -> bool:
    if (path / ".privibe").exists():
        return True
    if has_agents_md_file(path):
        return True
    return has_config_dirs_nearby(path, max_depth=max_depth)
