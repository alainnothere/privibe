...
# Module-level flag set by the agent loop before each LLM call when
# `llm_debug_dump` is enabled. Re-exported from `_debug_dump` so existing
# callers keep working; the shared module is the single source of truth.
from privibe.core.llm.backend._debug_dump import (  # noqa: E402
    dump_payload as _shared_dump_payload,
    is_enabled as _shared_is_enabled,
    set_debug_dump_payload,
)

# This is used only to communicate with anthropic... not for llama.cpp
...
class AnthropicAdapter(APIAdapter):
    ...
    def prepare_request(  # noqa: PLR0913
        ...
        *,
        ...
        # DEBUG LLM COMMUNICATIONS
        if _shared_is_enabled():
            _shared_dump_payload(body, payload)
