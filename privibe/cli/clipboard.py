...
_COPY_METHODS: list[Callable[[str], None]] = [
    *[fn for cmd, fn in _CMD_STRATEGIES if _has_cmd(cmd)],
    _copy_pyperclip,
    _copy_osc52,  # last resort: SSH/headless where no native clipboard tool is available
]
...
def is_reliable_clipboard_available() -> bool:
    """Return True if a native clipboard tool (xclip, wl-copy, pbcopy) is installed."""
    return any(_has_cmd(cmd) for cmd, _ in _CMD_STRATEGIES)
...
def copy_selection_to_clipboard(app: App, show_toast: bool = True) -> str | None:
    ...
    if not selected_texts:
        if show_toast:
            try:
                has_textual_selection = bool(app.screen.selections)
            except Exception:
                has_textual_selection = False
            if has_textual_selection:
                app.notify(
                    "Could not read selection. Install xclip or disable auto-copy via /autocopy.",
                    severity="warning",
                    timeout=5,
                )
        return None
