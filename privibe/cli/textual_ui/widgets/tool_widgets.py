...
from privibe.cli.textual_ui.ansi_markdown import AnsiMarkdown as Markdown
from privibe.cli.textual_ui.widgets.no_markup_static import NoMarkupStatic
from privibe.core.tools.builtins.ask_user_question import AskUserQuestionResult
from privibe.core.tools.builtins.bash import BashArgs, BashResult
from privibe.core.tools.builtins.grep import GrepArgs, GrepResult
from privibe.core.tools.builtins.read_file import ReadFileArgs, ReadFileResult
...
class GrepResultWidget(ToolResultWidget[GrepResult]):
    def compose(self) -> ComposeResult:
        ...
        if not self.result or not self.result.matches:
            yield from self._footer()
            return
        max_lines = self.preview_lines if self.collapsed else None
        if max_lines:
...
def get_result_widget(
    ...
    return widget_class(result, success, message, collapsed, warnings, preview_lines)
