...
Rules:
- Use only commits that touch the `privibe` folder in this repo since version {current_version}. Inspect git history to list relevant changes.
...
- Remove any subsection that has no bullets (leave no empty ### Added / ### Changed / etc)."""
    try:
        result = subprocess.run(
            ["privibe", "-p", prompt], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
...
def fill_whats_new_message(new_version: str) -> None:
    whats_new_path = Path("privibe/whats_new.md")
    ...
    prompt = f"""Fill privibe/whats_new.md using only the CHANGELOG.md section for version {new_version}.
...
Examples:
  ...
  uv run scripts/bump_version.py patch    # 1.0.0 -> 1.0.1
    ...
    try:
        ...
        # Update privibe/core/__init__.py
        update_hard_values_files(
            "privibe/__init__.py",
