...
from privibe.core.config.harness_files import init_harness_files_manager
...
if __name__ == "__main__":
    ...
    from privibe.core.types import LLMChunk

    sys.path.insert(0, str(TESTS_ROOT))

    # Apply mocking before importing any privibe modules
    ...
    patch(
        "privibe.integrations.mistral.backend.MistralBackend.complete",
        side_effect=mock_complete,
    ).start()
    patch(
        "privibe.core.llm.backend.generic.GenericBackend.complete",
        side_effect=mock_complete,
    ).start()
    patch(
        "privibe.integrations.mistral.backend.MistralBackend.complete_streaming",
        side_effect=mock_complete_streaming,
    ).start()
    patch(
        "privibe.core.llm.backend.generic.GenericBackend.complete_streaming",
        side_effect=mock_complete_streaming,
    ).start()

    from privibe.acp.entrypoint import main
