...
import privibe.core.autocompletion.completers as completers_module
from privibe.core.autocompletion.completers import PathCompleter
from privibe.core.autocompletion.fuzzy import fuzzy_match as real_fuzzy_match


@pytest.fixture()
def file_tree(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    (tmp_path / "src" / "utils").mkdir(parents=True)
    (tmp_path / "src" / "utils" / "helpers.py").write_text("", encoding="utf-8")
...
def test_directory_prefix_can_match_from_a_nested_path_segment(
    ...
    (tmp_path / "privibe" / "myacp").mkdir(parents=True)
    (tmp_path / "privibe" / "myacp" / "entrypoint.py").write_text("", encoding="utf-8")
    monkeypatch.chdir(tmp_path)

    results = PathCompleter().get_completions("@acp/", cursor_pos=5)

    assert "@privibe/acp/entrypoint.py" in results
    assert "@privibe/myacp/entrypoint.py" not in results
...
def test_exact_path_query_ranks_children_ahead_of_unrelated_fuzzy_matches(
    ...
    (tmp_path / "zephyr" / "generators" / "tests" / "test_gen.py").write_text(
        "", encoding="utf-8"
    )
    (tmp_path / "zephyr" / "generators" / "prompts").mkdir(parents=True)
    (tmp_path / "zephyr" / "generators" / "prompts" / "base.txt").write_text(
        "", encoding="utf-8"
    )
