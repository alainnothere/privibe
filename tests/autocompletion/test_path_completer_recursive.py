...
def file_tree(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    (tmp_path / "privibe" / "acp" / "entrypoint.py").write_text("")
    (tmp_path / "privibe" / "acp" / "agent.py").write_text("")
    (tmp_path / "privibe" / "cli" / "autocompletion").mkdir(parents=True)
    (tmp_path / "privibe" / "cli" / "autocompletion" / "fuzzy.py").write_text("")
    (tmp_path / "privibe" / "cli" / "autocompletion" / "completers.py").write_text("")
...
def test_finds_files_recursively_by_filename(file_tree: Path) -> None:
    ...
    assert results[0] == "@privibe/acp/entrypoint.py"
...
def test_finds_multiple_matches_recursively(file_tree: Path) -> None:
    ...
    vibe_index = results.index("@privibe/cli/autocompletion/fuzzy.py")
...
def test_prioritizes_exact_path_matches(file_tree: Path) -> None:
    results = PathCompleter().get_completions("@privibe/acp/entrypoint", cursor_pos=20)
...
def test_finds_files_when_pattern_matches_directory_name(file_tree: Path) -> None:
    ...
    assert results == [
        "@privibe/acp/",
        "@privibe/acp/agent.py",
        "@privibe/acp/entrypoint.py",
        "@privibe/cli/autocompletion/completers.py",
        "@tests/autocompletion/",
        "@tests/autocompletion/test_fuzzy.py",
        "@privibe/cli/autocompletion/",
        "@privibe/cli/autocompletion/fuzzy.py",
    ]
