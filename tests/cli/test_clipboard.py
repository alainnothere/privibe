...
@patch("privibe.cli.clipboard._copy_to_clipboard")
...
def test_copy_selection_to_clipboard_multiple_widgets(mock_app: MagicMock) -> None:
    ...
    with patch("privibe.cli.clipboard._copy_to_clipboard") as mock_copy_to_clipboard:
        result = copy_selection_to_clipboard(mock_app)
...
def test_copy_to_clipboard_stops_after_verified_copy() -> None:
    ...
    with (
        patch("privibe.cli.clipboard._COPY_METHODS", [mock_first, mock_second]),
        patch("privibe.cli.clipboard._read_clipboard", return_value="hello"),
    ):
...
def test_copy_to_clipboard_tries_all_when_verify_fails() -> None:
    ...
    with (
        patch("privibe.cli.clipboard._COPY_METHODS", [mock_first, mock_second]),
        patch("privibe.cli.clipboard._read_clipboard", return_value=None),
    ):
        _copy_to_clipboard("hello")
...
def test_copy_to_clipboard_raises_when_all_strategies_raise() -> None:
    ...
    with (
        patch("privibe.cli.clipboard._COPY_METHODS", [mock_osc52, mock_pyperclip]),
...
def test_read_clipboard_returns_first_successful_reader() -> None:
    ...
    with patch(
        "privibe.cli.clipboard._READ_CLIPBOARD_METHODS", [mock_reader, mock_reader2]
    ):
...
def test_read_clipboard_falls_through_on_failure() -> None:
    ...
    with patch("privibe.cli.clipboard._READ_CLIPBOARD_METHODS", [failing]):
...
def test_read_clipboard_skips_failing_reader() -> None:
    ...
    with patch("privibe.cli.clipboard._READ_CLIPBOARD_METHODS", [failing, working]):
        assert _read_clipboard() == "hello"
...
def test_is_reliable_clipboard_available_returns_true_when_tool_found() -> None:
    with patch("shutil.which", side_effect=lambda cmd: "/usr/bin/xclip" if cmd == "xclip" else None):
        assert is_reliable_clipboard_available()


def test_is_reliable_clipboard_available_returns_false_when_no_tools() -> None:
    with patch("shutil.which", return_value=None):
        assert not is_reliable_clipboard_available()


def test_is_reliable_clipboard_available_true_for_wl_copy() -> None:
    with patch("shutil.which", side_effect=lambda cmd: "/usr/bin/wl-copy" if cmd == "wl-copy" else None):
        assert is_reliable_clipboard_available()


def test_copy_selection_notifies_when_textual_has_selection_but_extraction_fails(
    mock_app: MagicMock,
) -> None:
    """When Textual has a selection but _get_selected_texts returns empty, warn the user."""
    mock_app.query.return_value = []
    mock_app.screen.selections = {"widget": object()}

    result = copy_selection_to_clipboard(mock_app, show_toast=True)

    assert result is None
    mock_app.notify.assert_called_once()
    message = mock_app.notify.call_args[0][0]
    assert "xclip" in message
    assert "/autocopy" in message


def test_copy_selection_no_notify_when_textual_has_selection_but_show_toast_false(
    mock_app: MagicMock,
) -> None:
    """When show_toast=False, no notification even if Textual has a selection."""
    mock_app.query.return_value = []
    mock_app.screen.selections = {"widget": object()}

    result = copy_selection_to_clipboard(mock_app, show_toast=False)

    assert result is None
    mock_app.notify.assert_not_called()


def test_copy_selection_no_notify_when_no_textual_selection(
    mock_app: MagicMock,
) -> None:
    """No notification when screen.selections is empty (user just clicked)."""
    mock_app.query.return_value = []
    mock_app.screen.selections = {}
...
def test_copy_selection_no_notify_when_screen_raises(
    mock_app: MagicMock,
) -> None:
    """No crash or notification when app.screen is inaccessible."""
    mock_app.query.return_value = []
    mock_app.screen = MagicMock()
    type(mock_app.screen).selections = property(lambda self: (_ for _ in ()).throw(RuntimeError("no screen")))
...
def test_copy_methods_includes_available_commands() -> None:
    ...
    import privibe.cli.clipboard as mod
