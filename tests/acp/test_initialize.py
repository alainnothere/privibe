...
class TestACPInitialize:
    ...
    async def test_initialize(self, acp_agent_loop: VibeAcpAgentLoop) -> None:
        ...
        assert response.agent_info == Implementation(
            name="privibe", title="Privibe", version=__version__
        )
    ...
    async def test_initialize_with_terminal_auth(
        ...
        assert auth_method.id == "privibe-setup"
        assert auth_method.name == "Register your API Key"
        assert auth_method.description == "Register your API Key inside Privibe"
        ...
        assert terminal_auth_meta["label"] == "Privibe Setup"
