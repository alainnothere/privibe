...
from privibe.acp.utils import ToolOption
...
def _create_vibe_home_dir(tmp_path: Path, *sections: dict[str, Any]) -> Path:
    """Create a temporary privibe home directory with a minimal config file."""
    vibe_home = tmp_path / ".privibe"
    vibe_home.mkdir()
    ...
    if base_config_dict.get("models"):
        for model in base_config_dict["models"]:
            if model.get("name") == "mistral-privibe-cli-latest":
...
def vibe_home_grep_ask(tmp_path: Path) -> Path:
    """Create a temporary privibe home directory with grep configured to ask permission."""
...
async def get_acp_agent_loop_process(
    ...
    env["PRIVIBE_HOME"] = str(vibe_home)
...
class TestSessionUpdates:
    ...
    async def test_tool_call_update_structure(self, vibe_home_dir: Path) -> None:
        ...
        async for process in get_acp_agent_loop_process(
            ...
            assert tool_call.params.update.title == 'grep "auth" in .'
            assert (
