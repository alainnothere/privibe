...
from privibe.acp.acp_agent_loop import VibeAcpAgentLoop
from privibe.core.agent_loop import AgentLoop
from privibe.core.config import ModelConfig, VibeConfig
...
def acp_agent_loop(backend) -> VibeAcpAgentLoop:
    ...
    patch("privibe.acp.acp_agent_loop.AgentLoop", side_effect=PatchedAgentLoop).start()
...
class TestACPSetModel:
    ...
    async def test_set_model_saves_to_config(
        ...
        with patch("privibe.acp.acp_agent_loop.VibeConfig.save_updates") as mock_save:
            response = await acp_agent_loop.set_session_model(
                session_id=session_id, model_id="devstral-small"
            )
    ...
    async def test_set_model_preserves_conversation_history(
        ...
        acp_session.agent_loop.messages.add(user_msg)
        acp_session.agent_loop.messages.add(assistant_msg)
