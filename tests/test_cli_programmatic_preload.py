...
from privibe.core import run_programmatic
from privibe.core.types import LLMMessage, OutputFormat, Role
...
def test_run_programmatic_preload_streaming_is_batched(
    ...
    monkeypatch.setattr(
        "privibe.core.programmatic.create_formatter", lambda *_args, **_kwargs: spy
    )
    ...
    with mock_backend_factory(
        ...
        # prepare_messages_for_resume injects a context_refresh user message before
        # the live prompt, so the sequence gains one extra user turn at index 3.
        assert roles == [
            Role.system,
            Role.user,
            Role.assistant,
            Role.user,  # context_refresh injected tail
            Role.user,
            Role.assistant,
        ]
        ...
        assert spy.emitted[4][1] == "Can you summarize what decorators are?"
        assert (
            spy.emitted[5][1]
...
def test_run_programmatic_ignores_system_messages_in_previous(
    ...
    with mock_backend_factory(
        ...
        roles = [r for r, _ in spy.emitted]
        # prepare_messages_for_resume injects a context_refresh user message after
