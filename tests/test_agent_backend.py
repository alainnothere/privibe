...
from privibe.core.tools.base import BaseToolConfig, ToolPermission
from privibe.core.types import EntrypointMetadata, FunctionCall, ToolCall
...
def _two_model_vibe_config(active_model: str) -> VibeConfig:
    ...
    models = [
        ModelConfig(
            name="mistral-privibe-cli-latest", provider="mistral", alias="devstral-latest"
        ),
        ModelConfig(
...
async def test_mcp_sampling_handler_uses_updated_config_when_agent_config_changes():
    ...
    result1 = await handler(context, params)
    assert isinstance(result1, CreateMessageResult)
    assert result1.model == "mistral-privibe-cli-latest"
