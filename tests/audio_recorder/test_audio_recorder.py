...
from privibe.core.audio_recorder.audio_recorder import AudioRecorder
from privibe.core.audio_recorder.audio_recorder_port import (
...
class TestBufferMode:
    @patch("privibe.core.audio_recorder.audio_recorder.sd.RawInputStream")
...
class TestGuardAudioInput:
    def test_guard_returns_sample_rate_when_compatible(self) -> None:
        with (
            patch(
                "privibe.core.audio_recorder.audio_recorder.sd.query_devices"
            ) as mock_query,
            patch("privibe.core.audio_recorder.audio_recorder.sd.check_input_settings"),
        ):
    ...
    def test_guard_raises_with_fallback_when_rate_incompatible(self) -> None:
        with (
            ...
            patch(
                "privibe.core.audio_recorder.audio_recorder.sd.check_input_settings",
                side_effect=sd.PortAudioError(-1),
            ),
        ):
    ...
    def test_start_raises_when_no_sounddevice(self) -> None:
        with patch("privibe.core.audio_recorder.audio_recorder.sd", None):
            recorder = AudioRecorder()
