...
def mock_log_dir(tmp_path: Path):
    ...
    mock_file = MagicMock()
    mock_file.path = tmp_path / "privibe.log"
    with (
        patch("privibe.core.logger.LOG_DIR", mock_dir),
        patch("privibe.core.logger.LOG_FILE", mock_file),
    ):
        yield tmp_path
...
class TestApplyLoggingConfig:
    def test_adds_handler_to_logger(
        ...
        # File handler + stderr handler.
        assert len(test_logger.handlers) == initial_handler_count + 2
    ...
    def test_creates_log_file(
        ...
        log_file = mock_log_dir / "privibe.log"
    ...
    def test_creates_log_directory_if_missing(
        ...
        mock_file = MagicMock()
        mock_file.path = log_dir / "privibe.log"
        with (
            patch("privibe.core.logger.LOG_DIR", mock_dir),
            patch("privibe.core.logger.LOG_FILE", mock_file),
        ):
    ...
    def test_log_max_bytes_from_env(
        ...
        # Verify the file handler was added with correct maxBytes
        handler = next(
