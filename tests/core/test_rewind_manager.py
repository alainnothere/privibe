...
from privibe.core.rewind.manager import FileSnapshot, RewindError, RewindManager
from privibe.core.types import LLMMessage, Role


def _make_messages(*contents: str) -> ConversationList:
    """Create a ConversationList with a system message followed by user/assistant pairs."""
    msgs: ConversationList = ConversationList()
    msgs.add(LLMMessage(role=Role.system, content="system"))
    for content in contents:
        msgs.add(LLMMessage(role=Role.user, content=content))
        msgs.add(LLMMessage(role=Role.assistant, content=f"reply to {content}"))
    return msgs
...
class TestRewind:
    ...
    def test_get_rewindable_messages_excludes_injected(self) -> None:
        ...
        messages.add(LLMMessage(role=Role.user, content="world"))
        messages.add(LLMMessage(role=Role.assistant, content="reply to world"))
        mgr, _, _ = _make_manager(messages)
...
class _Turn:
    ...
    def __init__(self, mgr: RewindManager, messages: ConversationList) -> None:
        self._mgr = mgr
        self._messages = messages

    def begin(self, user_msg: str) -> None:
        """Start a new turn: create checkpoint then add user message.
        ...
        *before* the user message is added to the message list.
        ...
        self._messages.add(LLMMessage(role=Role.user, content=user_msg))
    ...
    def end(self, assistant_reply: str = "ok") -> None:
        """End the turn: add assistant reply."""
        self._messages.add(LLMMessage(role=Role.assistant, content=assistant_reply))


@pytest.mark.asyncio
class TestRewindScenarios:
    @staticmethod
    def _setup() -> tuple[RewindManager, ConversationList, _Turn]:
        messages = ConversationList()
        messages.add(LLMMessage(role=Role.system, content="system"))
        mgr, _, _ = _make_manager(messages)
    ...
    async def test_rewind_restores_errors_collected(self, tmp_path: Path) -> None:
        ...
        turn1_idx = mgr.get_rewindable_messages()[0][0]
        with patch(
            "privibe.core.rewind.manager.os.remove",
