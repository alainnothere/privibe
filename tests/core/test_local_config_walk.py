...
class TestBoundedWalk:
    ...
    def test_respects_dir_count_limit(self, tmp_path: Path) -> None:
        ...
        (tmp_path / "zzz_last" / ".privibe" / "tools").mkdir(parents=True)
    ...
    def test_skips_ignored_directories(self, tmp_path: Path) -> None:
        (tmp_path / "node_modules" / ".privibe" / "tools").mkdir(parents=True)
        (tmp_path / ".privibe" / "tools").mkdir(parents=True)
        tools, _, _ = walk_local_config_dirs_all(tmp_path)
        assert tools == (tmp_path / ".privibe" / "tools",)

    def test_skips_dot_directories(self, tmp_path: Path) -> None:
        (tmp_path / ".hidden" / ".privibe" / "tools").mkdir(parents=True)
        tools, _, _ = walk_local_config_dirs_all(tmp_path)
        assert not tools

    def test_preserves_alphabetical_ordering(self, tmp_path: Path) -> None:
        (tmp_path / "bbb" / ".privibe" / "tools").mkdir(parents=True)
        (tmp_path / "aaa" / ".privibe" / "tools").mkdir(parents=True)
        (tmp_path / ".privibe" / "tools").mkdir(parents=True)
        tools, _, _ = walk_local_config_dirs_all(tmp_path, max_depth=1)
        assert tools == (
            tmp_path / ".privibe" / "tools",
            tmp_path / "aaa" / ".privibe" / "tools",
            tmp_path / "bbb" / ".privibe" / "tools",
        )
    ...
    def test_finds_all_config_types(self, tmp_path: Path) -> None:
        ...
        tools, skills, agents = walk_local_config_dirs_all(tmp_path)
        assert tmp_path / ".privibe" / "tools" in tools
        assert tmp_path / ".privibe" / "skills" in skills
        assert tmp_path / ".privibe" / "agents" in agents
        assert tmp_path / ".agents" / "skills" in skills
...
class TestHasConfigDirsNearby:
    ...
    def test_returns_true_for_shallow_nested(self, tmp_path: Path) -> None:
        (tmp_path / "sub" / ".privibe" / "skills").mkdir(parents=True)
        assert has_config_dirs_nearby(tmp_path, max_depth=1) is True

    def test_returns_true_at_depth_2(self, tmp_path: Path) -> None:
        (tmp_path / "a" / "b" / ".agents" / "skills").mkdir(parents=True)
        assert has_config_dirs_nearby(tmp_path, max_depth=2) is True

    def test_returns_false_beyond_default_depth(self, tmp_path: Path) -> None:
        (tmp_path / "a" / "b" / "c" / "d" / "e" / ".privibe" / "tools").mkdir(parents=True)
        assert has_config_dirs_nearby(tmp_path) is False
