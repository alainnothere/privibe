...
class TestProjectToolsDirs:
    ...
    def test_returns_empty_when_not_trusted(
        ...
        monkeypatch.chdir(tmp_path)
        monkeypatch.setattr(trusted_folders_manager, "is_trusted", lambda _: False)
        (tmp_path / ".privibe" / "tools").mkdir(parents=True)
        mgr = HarnessFilesManager(sources=("user", "project"))
        assert mgr.project_tools_dirs == []
    ...
    def test_returns_path_when_tools_dir_exists_and_trusted(
        ...
        monkeypatch.chdir(tmp_path)
        monkeypatch.setattr(trusted_folders_manager, "is_trusted", lambda _: True)
        tools_dir = tmp_path / ".privibe" / "tools"
    ...
    def test_ignores_tools_when_file_not_dir(
        ...
        monkeypatch.chdir(tmp_path)
        monkeypatch.setattr(trusted_folders_manager, "is_trusted", lambda _: True)
        (tmp_path / ".privibe").mkdir()
        (tmp_path / ".privibe" / "tools").write_text("", encoding="utf-8")
        mgr = HarnessFilesManager(sources=("user", "project"))
        assert mgr.project_tools_dirs == []
    ...
    def test_finds_tools_dirs_recursively(
        ...
        monkeypatch.setattr(trusted_folders_manager, "is_trusted", lambda _: True)
        (tmp_path / ".privibe" / "tools").mkdir(parents=True)
        (tmp_path / "sub" / ".privibe" / "tools").mkdir(parents=True)
        mgr = HarnessFilesManager(sources=("user", "project"), project_scan_depth=1)
        assert mgr.project_tools_dirs == [
            tmp_path / ".privibe" / "tools",
            tmp_path / "sub" / ".privibe" / "tools",
        ]
    ...
    def test_does_not_descend_into_ignored_dirs(
        ...
        monkeypatch.setattr(trusted_folders_manager, "is_trusted", lambda _: True)
        (tmp_path / ".privibe" / "tools").mkdir(parents=True)
        (tmp_path / ".git" / ".privibe" / "tools").mkdir(parents=True)
        mgr = HarnessFilesManager(sources=("user", "project"))
        assert mgr.project_tools_dirs == [tmp_path / ".privibe" / "tools"]
...
class TestProjectAgentsDirs:
    ...
    def test_returns_empty_when_not_trusted(
        ...
        monkeypatch.chdir(tmp_path)
        monkeypatch.setattr(trusted_folders_manager, "is_trusted", lambda _: False)
        (tmp_path / ".privibe" / "agents").mkdir(parents=True)
        mgr = HarnessFilesManager(sources=("user", "project"))
        assert mgr.project_agents_dirs == []
    ...
    def test_returns_path_when_agents_dir_exists_and_trusted(
        ...
        monkeypatch.chdir(tmp_path)
        monkeypatch.setattr(trusted_folders_manager, "is_trusted", lambda _: True)
        agents_dir = tmp_path / ".privibe" / "agents"
    ...
    def test_ignores_agents_when_file_not_dir(
        ...
        monkeypatch.setattr(trusted_folders_manager, "is_trusted", lambda _: True)
        (tmp_path / ".privibe").mkdir()
        (tmp_path / ".privibe" / "agents").write_text("", encoding="utf-8")
        mgr = HarnessFilesManager(sources=("user", "project"))
        assert mgr.project_agents_dirs == []
    ...
    def test_finds_agents_dirs_recursively(
        ...
        monkeypatch.setattr(trusted_folders_manager, "is_trusted", lambda _: True)
        (tmp_path / ".privibe" / "agents").mkdir(parents=True)
        (tmp_path / "sub" / "deep" / ".privibe" / "agents").mkdir(parents=True)
        mgr = HarnessFilesManager(sources=("user", "project"), project_scan_depth=2)
        assert mgr.project_agents_dirs == [
            tmp_path / ".privibe" / "agents",
            tmp_path / "sub" / "deep" / ".privibe" / "agents",
        ]
    ...
    def test_does_not_descend_into_ignored_dirs(
        ...
        monkeypatch.setattr(trusted_folders_manager, "is_trusted", lambda _: True)
        (tmp_path / ".privibe" / "agents").mkdir(parents=True)
        (tmp_path / "__pycache__" / ".privibe" / "agents").mkdir(parents=True)
        mgr = HarnessFilesManager(sources=("user", "project"))
        assert mgr.project_agents_dirs == [tmp_path / ".privibe" / "agents"]
...
class TestUserToolsDirs:
    ...
    def test_returns_empty_when_dir_does_not_exist(
        ...
        monkeypatch.setattr("privibe.core.paths._vibe_home._DEFAULT_VIBE_HOME", tmp_path)
        mgr = HarnessFilesManager(sources=("user",))
        assert mgr.user_tools_dirs == []
...
class TestProjectSkillsDirs:
    ...
    def test_returns_vibe_skills_only_when_only_it_exists(
        ...
        monkeypatch.chdir(tmp_path)
        monkeypatch.setattr(trusted_folders_manager, "is_trusted", lambda _: True)
        vibe_skills = tmp_path / ".privibe" / "skills"
    ...
    def test_ignores_vibe_skills_when_file_not_dir(
        ...
        monkeypatch.setattr(trusted_folders_manager, "is_trusted", lambda _: True)
        (tmp_path / ".privibe").mkdir()
        (tmp_path / ".privibe" / "skills").write_text("", encoding="utf-8")
        mgr = HarnessFilesManager(sources=("user", "project"))
        assert mgr.project_skills_dirs == []
    ...
    def test_finds_skills_dirs_recursively_in_trusted_folder(
        ...
        monkeypatch.chdir(tmp_path)
        monkeypatch.setattr(trusted_folders_manager, "is_trusted", lambda _: True)
        (tmp_path / ".privibe" / "skills").mkdir(parents=True)
        (tmp_path / "sub" / ".agents" / "skills").mkdir(parents=True)
        (tmp_path / "sub" / "deep" / ".privibe" / "skills").mkdir(parents=True)
        mgr = HarnessFilesManager(sources=("user", "project"), project_scan_depth=2)
        assert mgr.project_skills_dirs == [
            tmp_path / ".privibe" / "skills",
            tmp_path / "sub" / ".agents" / "skills",
            tmp_path / "sub" / "deep" / ".privibe" / "skills",
        ]
    ...
    def test_does_not_descend_into_ignored_dirs(
        ...
        monkeypatch.setattr(trusted_folders_manager, "is_trusted", lambda _: True)
        (tmp_path / ".privibe" / "skills").mkdir(parents=True)
        (tmp_path / "node_modules" / ".privibe" / "skills").mkdir(parents=True)
        mgr = HarnessFilesManager(sources=("user", "project"))
        assert mgr.project_skills_dirs == [tmp_path / ".privibe" / "skills"]
