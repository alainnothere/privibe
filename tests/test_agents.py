...
class TestAgentApplyToConfig:
    ...
    def test_custom_prompt_found_in_global_when_missing_from_project(
        ...
        ~/.privibe/prompts/ directory and load the file from there.
        """
        project_prompts = tmp_path / "project" / ".privibe" / "prompts"
        project_prompts.mkdir(parents=True)

        global_prompts = tmp_path / "home" / ".privibe" / "prompts"
        ...
        monkeypatch.setattr(
            "privibe.core.config._settings.get_harness_files_manager", lambda: mock_manager
        )
...
class TestAgentSwitchAgent:
    ...
    async def test_switch_agent_preserves_conversation_history(
        ...
        agent.messages.add(user_msg)
        agent.messages.add(assistant_msg)
...
class TestAgentLoopInitialization:
    def test_agent_system_prompt_id_is_applied_on_init(
        ...
        monkeypatch.setattr("privibe.core.agents.models.BUILTIN_AGENTS", patched_agents)
        monkeypatch.setattr("privibe.core.agents.manager.BUILTIN_AGENTS", patched_agents)
