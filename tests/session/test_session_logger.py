...
def mock_vibe_config() -> VibeConfig:
    """Create a mock privibe config for testing."""
...
class TestSessionLoggerMetadata:
    @patch("privibe.core.session.session_logger.subprocess.run")
    @patch("privibe.core.session.session_logger.getpass.getuser")
...
class TestSessionLoggerSaveInteraction:
    ...
    async def test_save_interaction_throttles_tmp_cleanup(
        ...
        with (
            ...
            patch(
                "privibe.core.session.session_logger.utc_now",
